    ...
    <div className="h-full w-full flex flex-col bg-slate-100 dark:bg-slate-950 overflow-hidden relative">
      <div className="p-2.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center z-20 shadow-sm">
        <div className="flex items-center gap-3">
           <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">{isMarkdown ? 'MARKDOWN SOURCE' : 'PLAINTEXT SOURCE'}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
           ...
           <button onClick={handleCopy} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${copied ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-900 dark:bg-slate-800 text-white shadow-lg'}`}>
