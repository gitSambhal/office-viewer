...
export const DocxViewer: React.FC<Props> = ({ data, name }) => {
    ...
    const renderDoc = async () => {
      ...
      try {
        ...
        if (viewMode === 'layout') {
          ...
          if (!renderer || !renderer.renderAsync) throw new Error('Layout engine missing.');
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`<html><head><title>${name || 'Document'}</title>${styles}<style>body { background: white !important; padding: 2cm; } .docx-inner, .prose { box-shadow: none !important; margin: 0 auto !important; }</style></head><body>${containerRef.current.innerHTML}</body></html>`);
  ...
  return (
    <div className="h-full w-full flex flex-col bg-zinc-100 dark:bg-zinc-950 overflow-hidden relative">
      ...
      <div className="flex-1 overflow-auto p-4 md:p-12 custom-scrollbar">
        ...
        <div ref={containerRef} className="max-w-5xl mx-auto docx-preview-container" />
