  ...
  return (
    <header className="hide-in-zen flex items-center justify-between px-4 py-2 sm:px-6 sm:py-3 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 select-none z-30 shadow-sm shrink-0 gap-2 sm:gap-4">
      <div className="flex items-center gap-3 sm:gap-6">
        <div title="Go to Dashboard" className="flex items-center gap-2.5 group cursor-pointer" onClick={() => dispatch({ type: 'SET_ACTIVE_TAB', payload: null })}>
        ...
        {state.activeTabId && (
          <nav className="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-lg sm:rounded-xl p-1 gap-1">
            <button title="Sidebar" onClick={() => dispatch({ type: 'SET_SIDEBAR_OPEN', payload: !state.isSidebarOpen })} className={`p-1.5 sm:p-2 rounded-md sm:rounded-lg transition-all ${state.isSidebarOpen ? 'bg-white dark:bg-zinc-700 shadow-md text-violet-600' : 'text-zinc-500 hover:bg-white/50 dark:hover:bg-zinc-700/50'}`}>
            ...
            <button title="Toggle Theme" onClick={() => dispatch({ type: 'SET_DARK_MODE', payload: !state.darkMode })} className="p-1.5 sm:p-2 rounded-md sm:rounded-lg hover:bg-white dark:hover:bg-zinc-700 transition-all text-zinc-500">
              {state.darkMode ? <IconLight /> : <IconDark />}
            </button>
            <button title="Zen Mode" onClick={() => dispatch({ type: 'SET_ZEN_MODE', payload: !state.zenMode })} className={`p-1.5 sm:p-2 rounded-md sm:rounded-lg hover:bg-white dark:hover:bg-zinc-700 transition-all ${state.zenMode ? 'text-violet-600 bg-white dark:bg-zinc-700 shadow-md' : 'text-zinc-500'}`}>
              <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
            </button>
            <button title="Fullscreen" onClick={toggleFullscreen} className="p-1.5 sm:p-2 rounded-md sm:rounded-lg hover:bg-white dark:hover:bg-zinc-700 transition-all text-zinc-500">
              <IconFullscreen />
            </button>
          </nav>
        )}
        ...
        <div className="hidden sm:block flex-1 max-w-md relative">
          <div className="relative">
            ...
            {state.globalSearchTerm && (
              <button
                onClick={() => dispatch({ type: 'SET_GLOBAL_SEARCH_TERM', payload: '' })}
