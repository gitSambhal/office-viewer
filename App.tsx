...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (viewerType) {
      // Update SEO based on viewer type
      updateSEO(null, undefined, viewerType);
    }
  }, []);
  ...
  const { handleFiles, isProcessing, errorMessage, setErrorMessage } =
    useFileHandler();
  ...
  const handleFilesRef = React.useRef<
    ((files: FileList | File[] | null) => void) | null
  >(null);
  ...
  const retrieveSharedFiles = React.useCallback(
    async (shareId: string): Promise<File[] | null> => {
      try {
        ...
        return new Promise((resolve, reject) => {
          ...
          request.onsuccess = () => {
            ...
            if (fileData && fileData.files) {
              const fileArray: File[] = fileData.files
                .map(
                  (f: {
                    name: string;
                    type: string;
              ...
              console.log(
                '[App] Retrieved files from IndexedDB:',
                fileArray.length,
  ...
  const clearSharedFiles = React.useCallback(
  ...
  useEffect(() => {
    ...
    const checkInitialSharedFiles = async () => {
      ...
      try {
        ...
        request.onsuccess = () => {
          const fileDataList = request.result as Array<{
            id: string;
            files: Array<{
              name: string;
              type: string;
          ...
          if (
            ...
            console.log(
              '[App] Found shared files on load:',
              fileArray.length,
              fileArray.map((f) => f.name)
            );
        ...
        request.onerror = () => {
          console.error(
            '[App] Error getting shared files from IndexedDB:',
            request.error
          );
        };
      } catch (err) {
    ...
    if (win.launchQueue) {
      ...
      win.launchQueue.setConsumer(async (params: any) => {
        console.log(
          '[App] launchQueue setConsumer called with params:',
          params
        );
    ...
    if (navigator.serviceWorker) {
      ...
      navigator.serviceWorker.ready
        .then((registration) => {
          console.log(
            '[App] Service worker is ready and controlling:',
    ...
    <ErrorBoundary>
      <div
        ...
        onDrop={(e) => {
          ...
          const entries = Array.from(e.dataTransfer.items).map((item) =>
            (item as any).webkitGetAsEntry?.()
          );

          const hasDirectories = entries.some(
            ...
            const processAllEntries = async () => {
              ...
              if (supportedFiles.length > 0) {
                ...
                const confirmed = window.confirm(
                  `Found ${supportedFiles.length} supported file${supportedFiles.length > 1 ? 's' : ''} in the dropped ${hasDirectories ? 'folder' : 'items'}. Do you want to open them?`
                );
                if (confirmed) {
                ...
                setErrorMessage(
                  'No supported files found in the dropped folder.'
                );
              }
            };
        ...
        <div className="dropzone-overlay">
          <div className="bg-white dark:bg-zinc-900 p-16 rounded-[4rem] shadow-2xl flex flex-col items-center gap-8 animate-in zoom-in duration-300 border-2 border-violet-100 dark:border-violet-900/30">
            ...
            <div className="text-center">
              <h2 className="text-4xl font-black tracking-tighter text-zinc-800 dark:text-white mb-2">
                Drop to Open
              </h2>
              <p className="text-zinc-500 dark:text-zinc-400 font-bold uppercase tracking-widest text-[11px]">
          ...
          <div className="fixed inset-0 bg-white/60 dark:bg-zinc-950/60 backdrop-blur-md z-[100] flex flex-col items-center justify-center animate-in fade-in duration-300">
            <div className="bg-white dark:bg-zinc-900 p-12 rounded-[3rem] shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col items-center text-center max-w-sm">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-xs font-bold uppercase tracking-widest leading-relaxed">
                Parsing local records. Large files may take a few seconds.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[110] animate-in slide-in-from-top-4 duration-300">
            <div className="bg-rose-600 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 max-w-lg border-2 border-rose-500">
              ...
              <button
                ...
                className="p-1 hover:bg-rose-700 rounded-lg transition-colors"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
