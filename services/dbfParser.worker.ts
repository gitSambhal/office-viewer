...
self.onmessage = async (e: MessageEvent<ParsingMessage>) => {
  ...
  try {
    ...
    // Report progress
    self.postMessage({ type: 'PROGRESS', progress: 0, stage: 'Parsing header...' });
    ...
    self.postMessage({ type: 'PROGRESS', progress: 10, stage: 'Parsing fields...' });
    ...
    self.postMessage({ type: 'PROGRESS', progress: 20, stage: 'Parsing records...' });
    ...
    while (processedRecords < numberOfRecords) {
            ...
            if (field.type === 'I') {
              ...
              if (field.length >= 8) {
                ...
                if (julianDay === 0) {
                  ...
                  value = isNaN(d.getTime()) ? "[Invalid Date]" : d.toISOString().replace('T', ' ').split('.')[0];
                }
              } else {
                value = null;
              }
      ...
      const progress = 20 + Math.floor((processedRecords / numberOfRecords) * 70);
      self.postMessage({ type: 'PROGRESS', progress, stage: `Parsed ${processedRecords}/${numberOfRecords} records...` });
