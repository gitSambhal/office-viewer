...
export class TextExtractor {
  ...
  private static extractFromExcel(
    sheets: Record<string, SheetData>,
    activeSheet?: string
  ): string {
    if (!sheets || Object.keys(sheets).length === 0)
      return 'The Excel file is empty or has no recognizable data.';
    ...
    for (const [sheetName, sheetData] of Object.entries(sheetsToProcess)) {
      ...
      if (colCount > 25) {
        ...
        fullText +=
          (sheetData.headers.join(' | ') || '(No headers found)') + '\n';
      }
      ...
      if (rowCount > 0) {
        const sheetText = sheetData.rows
          ...
          .map(
            (row) =>
              row.slice(0, 15).join(' | ') +
              (row.length > 15 ? ` ... [+${row.length - 15} more cells]` : '')
          )
          .join('\n');
    ...
    const finalOutput =
      fullText.trim() ||
      'No text content could be extracted from this Excel file.';
    return finalOutput.substring(0, MAX_EXTRACT_LENGTH);
  }

  private static async extractFromMdb(
    file: File | any,
    activeTable?: string
  ): Promise<string> {
    try {
  ...
  private static async extractFromSqlite(
    file: File | any,
    activeTable?: string
  ): Promise<string> {
    try {
  ...
  private static extractFromDatabase(
    data: any,
